 ...
 */
class CreatePost extends \Magento\Customer\Controller\Account\CreatePost implements CsrfAwareActionInterface, HttpPostActionInterface
{
    /**
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($confirmationStatus === AccountManagementInterface::ACCOUNT_CONFIRMATION_REQUIRED) {
                ...
                $url = $this->urlModel->getUrl('*/*/createsuccess', ['_secure' => true]);
                $resultRedirect->setUrl($this->_redirect->success($url));
            } else {
                $this->session->setCustomerDataAsLoggedIn($customer);
                $requestedRedirect = $this->accountRedirect->getRedirectCookie();
                if (!$this->scopeConfig->getValue('customer/startup/redirect_dashboard') && $requestedRedirect) {
                    $resultRedirect->getUrl('*/*/createsuccess', ['_secure' => true]);
